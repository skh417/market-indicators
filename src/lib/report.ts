...
입력 JSON은 미국·한국 증시 지표 6종의 현재값과 압축 통계입니다.
...
분석 원칙:
1. 지표를 세 축으로 묶어 교차 해석할 것: 밸류에이션(버핏 지수·CAPE), 심리(VIX·VKOSPI·공포탐욕), 수급(코스피 투자자별).
...
3. '심리와 수급' — 미국·한국 변동성과 공포탐욕, 외국인·기관·개인 수급 흐름.
4. '주목할 포인트' — 상충 신호, 구간 경계에 근접한 지표(현재값이 어느 기준선에 가까운지), 앞으로 확인할 변화.
5. '유의사항' — 정보 제공 목적이며 투자 조언이 아님을 한두 문장으로.
